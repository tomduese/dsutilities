...
def scale_columns(df_train, df_test, col_list=[], scale_type=StandardScaler()):
    ...
    scaled_columns_train = pd.DataFrame(scale_type.fit_transform(df_train[col_list]),columns = col_list, index=df_train.index)
    scaled_columns_test = pd.DataFrame(scale_type.transform(df_test[col_list]),columns = col_list, index=df_test.index)

    df_train_preprocessed = pd.concat([scaled_columns_train, df_train.drop(col_list, axis=1)], axis=1)
    df_test_preprocessed = pd.concat([scaled_columns_test, df_test.drop(col_list, axis=1)], axis=1)
